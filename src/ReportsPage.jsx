...
const ReportsPage = () => {
  ...
  const [mainWindowLoaded, setMainWindowLoaded] = useState(false);
  ...
  const sendtoBackend = async () => {
    try {
      ...
      axios
        .post("http://localhost:5000/submit_selected_options", formData)
        .then((response) => {
          const JsonData = JSON.parse(response.data.data);
            ...
            <tbody>
              {studentData.map((val, idx) => (
                <tr key={idx}>
                  ...
                  <td key={"Exam Roll Number" + idx}>
                    {val["Exam Roll Number"]}
                  </td>
